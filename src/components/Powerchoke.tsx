  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-0 md:p-4">
      <div className="bg-slate-900 w-full h-full md:h-auto md:max-w-4xl md:rounded-2xl border-0 md:border border-slate-700 shadow-2xl flex flex-col md:max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-4 md:p-6 border-b border-slate-700 flex justify-between items-start bg-slate-800/50 md:rounded-t-2xl">
            ...
            <div className="flex items-center gap-2 md:gap-3 mb-2 flex-wrap">
              ...
              {company.headquarters && (
                <div className="flex items-center gap-1 text-[10px] md:text-xs text-slate-500">
                  ...
                  <span className="truncate max-w-[100px] md:max-w-[180px]">{company.headquarters}</span>
                </div>
              )}
            </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              className="p-2 hover:bg-slate-700 rounded-full transition-colors print:hidden"
              title="Print company profile"
            >
              <Printer className="w-5 h-5 md:w-6 md:h-6 text-slate-400" />
            </button>
            <button
              onClick={onClose}
              className="p-2 hover:bg-slate-700 rounded-full transition-colors print:hidden"
        ...
        <div className="hidden print:block px-6 py-3 border-b border-slate-300 bg-white">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              <span className="font-bold text-lg text-slate-900">COMPANY PROFILE</span>
            </div>
            <span className="text-sm text-slate-600">powerchoke.com</span>
          </div>
        </div>

        {/* BODY */}
        <div className="p-4 md:p-8 overflow-y-auto flex-1 print:overflow-visible print:p-6">
          {/* SCORES SECTION */}
        ...
        <div className="hidden print:block px-6 py-3 border-t border-slate-300 bg-white mt-auto">
          <div className="flex justify-between items-center text-xs text-slate-600">
            <div className="flex items-center gap-4">
              <span>
                Data as of: {company.data_updated || company.financial_ratings?.updated || 'N/A'}
              </span>
              {company.data_sources && company.data_sources.length > 0 && (
                <span>Sources: {company.data_sources.join(', ')}</span>
              )}
              {company.financial_ratings?.source && !company.data_sources?.includes(company.financial_ratings.source) && (
                <span>Financial data: {company.financial_ratings.source}</span>
              )}
            </div>
            <span>Generated by powerchoke.com</span>
          </div>
        </div>
      </div>
    </div>
